...
class MailComposer(models.TransientModel):
    ...
    """
    _name = 'mail.compose.message.event'
    ...
    attachment_ids = fields.Many2many(
        'ir.attachment', 'mail_compose_message_event_ir_attachments_rel',
    ...
    @api.multi
    def batch_send_action(self):
        mail_obj = self.env['mail.mail']
        emails_list=self.emails.split(",")
        msg_id = mail_obj.sudo().create({'body_html':self.body,'email_to':self.emails,'attachment_ids':[(4,id) for id in self.attachment_ids.ids],'subject':self.subject})
        event=self.env['event.event'].sudo().search([('id','=',self.res_id)])
        msg_id.send()

        if msg_id.state=='sent':
            event.sudo().write({'attendee_ids':[(1,id,{'agenda_sent':True}) for id in event.attendee_ids.ids]})
            event.sudo().write({'employee_ids':[(1,id,{'agenda_sent':True}) for id in event.employee_ids.ids]})
            event.sudo().write({'guest_ids':[(1,id,{'agenda_sent':True}) for id in event.guest_ids.ids]})
        #event.write({})
        '''
        for email in emails_list:
            msg_id = mail_obj.create({'body_html':self.body,'email_to':self.emails,'attachment_ids':[(4,id) for id in self.attachment_ids.ids]})
