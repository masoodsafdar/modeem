...
class MkTestInternalRegisterations(models.Model):
    ...
    def _test_time(self):
        ...
        for rec in self:
            ...
            if rec.period_id:
                if rec.subh:
                	time = 'من' +  str(rec.period_id.subh_period_from) +' ' +'الى' + str(rec.period_id.subh_period_to)  
                elif rec.zuhr:
                	time = 'من' +  str(rec.period_id.zuhr_period_from) +' ' +'الى' + str(rec.period_id.zuhr_period_to)  
                elif rec.subh:
                	time = 'من' +  str(rec.period_id.aasr_period_from) +' ' +'الى' + str(rec.period_id.aasr_period_to)  
                elif rec.magrib:
                	time = 'من' +  str(rec.period_id.magrib_period_from) +' ' +'الى' + str(rec.period_id.magrib_period_to)  
                elif rec.esha:
                	time = 'من ' +  str(rec.period_id.esha_period_from) +' ' +'الى' + str(rec.period_id.esha_period_to)  
                rec.test_time = time	
            else:
                rec.test_time = time
    ...
    @api.onchange('test_center_config_id')
    def onchange_test_center(self):
        if self.test_center_config_id:
            return {'domain':{'test_type_id':[('id','in',self.test_center_config_id.test_type_ids.ids)]}}
        else:
            return {'domain':{'test_type_id':[('id','in',[])]}}
    ...
    test_branch_id = fields.Many2one('test.branch.line', string='Test Name')
    ...
    def onchage_test_type(self):
        return {'domain':{'test_branch_id':[('line_id.name', '=', self.test_type_id.name)]}}
    ...
    def create(self, vals):
        lst=[]
        obj_student_prep = self.env['mk.test.student.preparation']
        rec = super(MkTestInternalRegisterations, self).create(vals)
        
        obj_student_prep.create(
                    
            {
              'company_id':rec['company_id'].id,
              'test_type_id':rec['test_type_id'].id,
              'study_year_id':rec['study_year_id'].id,
              'study_class_id':rec['study_class_id'].id,
              'test_center_config_id':rec['test_center_config_id'].id,
              'center_department_id':rec['center_department_id'].id,
              'mosque_department_id':rec['mosque_department_id'].id,
              'test_registeration_id':rec.id,
              'link_id':rec['student_id'].id,
              'mosque_id':rec['mosque_id'].id,
              'period_t':rec['period_id'].id,
