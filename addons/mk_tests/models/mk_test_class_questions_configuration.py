...
class MkTestClassQuestionsConfiguration(models.Model):
    ...
    study_class_id = fields.Many2one('mk.study.class', string='Study Class Year', default=get_study_class,)
...
class MkTestClassParts(models.Model):
    ...
    def get_question_numbers(self):
        for rec in self :
            rec.question_number = (rec.easy or 0) + (rec.middle or 0) + (rec.difficult or 0)

    @api.onchange('easy','middle','difficult','part_id,question_number')
    @api.constrains('easy','middle','difficult','part_id')
    def question_number_constraints(self):
        if self.question_number > self.test_question_id.question_number:
            raise ValidationError(_('Invalid Number of Questions Number'))
    ...
    test_question_id = fields.Many2one('mk.test.class', string='test question', ondelete="cascade")
    ...
    question_number = fields.Float(compute='get_question_numbers', string='Number of Questions Per Part')
